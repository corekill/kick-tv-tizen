(function(){
  ...
  var offlinePollTimer=null,streamRequestSeq=0,avPlaySequence=0;
  ...
  function avErrorText(error){
    if(error===undefined||error===null)return tr('unknown');
  ...
  function startAVPlay(url,roomId,keepChat){
    ...
    try{
      ...
      webapis.avplay.setListener({
        ...
        onbufferingprogress:function(percent){if(active())document.getElementById('statusDetail').textContent='Buffer '+percent+' %';},
        onbufferingcomplete:function(){if(active()){revealVideo();showHud();}},
        oncurrentplaytime:function(){if(active()&&!view.classList.contains('player-ready'))revealVideo();},
        onstreamcompleted:function(){if(active())showOfflineChannel({user:{username:currentChannelDisplayName},user_id:currentKickUserId,chatroom:{id:currentRoomId}},currentChannelName);},
        onerror:function(error){if(active())setStatus(tr('playbackError'),avErrorText(error),true);},
        onevent:function(){},onsubtitlechange:function(){},ondrmevent:function(){}
      });
      webapis.avplay.prepareAsync(function(){if(!active())return;try{webapis.avplay.setDisplayRect(0,0,1920,1080);webapis.avplay.play();}catch(e){failed(e,tr('cannotPlay'));}},function(error){failed(error,tr('cannotPrepare'));});
    }catch(e){failed(e,tr('avFailed'));}
  }
